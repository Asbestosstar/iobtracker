...
public class TrackerProperties implements ClampedItemPropertyFunction {
	...
	public float unclampedCall(ItemStack p_174672_, @Nullable ClientLevel p_174673_, @Nullable LivingEntity p_174674_,
			int p_174675_) {
		Entity entity = p_174674_ != null ? p_174674_ : p_174672_.getEntityRepresentation();
		if (entity == null) {
			return 0.0F;
		} else {
			...
			if (blockpos != null && !(entity.position().distanceToSqr(blockpos.getX() + 0.5D,
				...
				double d1 = 0.0D;
				if (flag) {
					d1 = p_174674_.getYRot();
				} else if (entity instanceof ItemEntity) {
					d1 = 180.0F - ((ItemEntity) entity).getSpin(0.5F) / ((float) Math.PI * 2F) * 360.0F;
				} else if (p_174674_ != null) {
					d1 = p_174674_.yBodyRot;
				}

				d1 = Mth.positiveModulo(d1 / 360.0D, 1.0D);
				double d2 = getAngleTo(Vec3.atCenterOf(blockpos), entity) / ((float) Math.PI * 2F);
				double d3;
				if (flag) {
				...
				double d0 = this.wobbleRandom.rotation + hash(p_174675_) / 2.14748365E9F;
